...
def aguardar_adc_5v(ser, const_fonte, red_ADCs, tempo_max=23):
...
def testar_dcdc_e_carga(ser, const_fonte, red_cf, red_DCDC, red_batt, red_load, file_name_txt):
    try:
        ...
        if (ADC_DCDC > 22) and (11 < ADC_cf < 15):
            Teste1B = True
            info_status = 'OK'
...
def main():
    ...
    try:
        ...
        if 'ser' in locals() and ser.is_open:
            # Sequencia de desligamento de cargas e fontes
            ser.write(b'DESDC\r')
            aguardar_ack(ser)
            ser.write(b'DESBT\r')
            aguardar_ack(ser)
            ...
            # ser.write(b"$rBoardVersion\r")       # envia o comando
            # resp = ser.read_until(b"\r").decode().strip()  # lê até '\n' e converte p/ string
